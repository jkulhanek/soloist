...
class SoloistConversation(Conversation):
...
class SoloistConversationalPipeline(transformers.Pipeline):
    """
    Multi-turn conversational pipeline.

    This conversational pipeline can currently be loaded from :func:`~transformers.pipeline` using the following
    task identifier: :obj:`"soloist-conversational"`.

    SoloistConversationalPipeline is similar to `transformers.ConversationalPipeline`,
    ...
    def __call__(self, conversations: Union[SoloistConversation, Conversation, List[Union[SoloistConversation, Conversation]]]):
        r"""
        Generate responses for the conversation(s) given as inputs.

        Args:
            conversations (a :class:`~transformers.Conversation` or a list of :class:`~transformers.Conversation`):
                Conversations to generate responses for. If `SoloistConversation` instances are passed as the input,
        ...
        if isinstance(conversations, list):
            for conversation in conversations:
                ...
                ), "SoloistConversationalPipeline expects a Conversation or list of Conversations as an input"
            ...
            raise ValueError("SoloistConversationalPipeline expects a Conversation or list of Conversations as an input")

        with self.device_placement():
            contexts = SoloistConversationalPipeline._get_contexts_from_conversations(conversations)
...
transformers.pipelines.SUPPORTED_TASKS["soloist-conversational"] = {
    "impl": SoloistConversationalPipeline,
...
# Utility function for transformers to call `pipeline('soloist-conversational')` with default model.
__old_pipeline = transformers.pipeline
def soloist_pipeline(task: str, model: Optional = None, *args, **kwargs) -> transformers.Pipeline:  # noqa
    if task == 'soloist-conversational':
...
transformers.pipeline = soloist_pipeline
transformers.pipelines.pipeline = soloist_pipeline
